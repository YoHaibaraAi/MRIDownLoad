...
> **重要隐私提醒：**本扩展下载的是**未脱敏的原始 DICOM**。影像文件及其元数据可能包含患者姓名、编号等个人信息；ZIP 文件名和序列目录也会使用检查日期、检查描述及序列描述，不能保证不含可识别信息。请只下载自己有权访问的检查，保存在可信设备上，不要将原始 ZIP、DICOM、分享链接或 HAR 上传到公开仓库或分享给无关人员。如需对外提供影像，请先按适用要求完成脱敏与核验。
...
6. 浏览器会保存一个 ZIP，例如 `20260101_STUDY_DICOM.zip`。实际文件名来自当前检查日期与描述，序列目录名来自序列编号与描述；这些描述可能包含个人信息。
...
- 扩展不会对 DICOM 文件内容、文件名或 ZIP 内目录做医疗信息脱敏；本地生成不等于文件不含敏感信息。
...
请勿把包含医疗数据的日志或 HAR 发到公开位置。`scripts/analyze-har.mjs` 的输出可能包含检查及序列描述，分享输出前也应检查并脱敏。
